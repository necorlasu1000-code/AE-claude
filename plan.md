...
- **5.1** ⏳ MVP 5 tool 직렬 — collocation 패턴 확립
  ...
  - **5.1.4 fix** ✅ mistakes #17 — ExtendScript this-binding 강제 dogfood 발견. impl.ts `project.item!(i)` 직접 호출 + `_mockApp.ts` receiver guard (30 tool 공통 mock policy) + regression case (본 commit)
  - **5.1.5~5.1.6** ⏳ 3 새 MVP tool 직렬 추가 (레이어/키프레임/이펙트 lane reference example 1개씩)
