...
- **5.1** ⏳ MVP 5 tool 직렬 — collocation 패턴 확립
  ...
  - **5.1.7** ✅ `ae_get_expression` (read-only, MVP 4/5 익스프레션 lane) — 4파일 collocation + `JsxPropertyLike` 확장 (expression?/expressionEnabled? optional) + `makeMockProperty` helper + `MockLayerOpts.properties` map (matchName→Property lookup) + propertyMatchName not-found → AENotFoundError 재사용 + 7 cases (본 commit)
